...
static void FLASH_OB_WRPConfig(uint32_t WRPArea, uint32_t WRPStartOffset,
...
static void FLASH_OB_PCROP1AConfig(uint32_t PCROPConfig,
		uint32_t PCROP1AStartAddr, uint32_t PCROP1AEndAddr);
static void FLASH_OB_PCROP1BConfig(uint32_t PCROP1BStartAddr,
		uint32_t PCROP1BEndAddr);
static void FLASH_OB_GetPCROP1A(uint32_t *PCROPConfig,
		uint32_t *PCROP1AStartAddr, uint32_t *PCROP1AEndAddr);
static void FLASH_OB_GetPCROP1B(uint32_t *PCROP1BStartAddr,
		uint32_t *PCROP1BEndAddr);
#if defined(FLASH_DBANK_SUPPORT)
static void FLASH_OB_PCROP2AConfig(uint32_t PCROP2AStartAddr,
		uint32_t PCROP2AEndAddr);
static void FLASH_OB_PCROP2BConfig(uint32_t PCROP2BStartAddr,
		uint32_t PCROP2BEndAddr);
static void FLASH_OB_GetPCROP2A(uint32_t *PCROP2AStartAddr,
		uint32_t *PCROP2AEndAddr);
static void FLASH_OB_GetPCROP2B(uint32_t *PCROP2BStartAddr,
		uint32_t *PCROP2BEndAddr);
    ...
    pEraseInit->Banks = FLASH_BANK_1;
		...
		if (pEraseInit->TypeErase == FLASH_TYPEERASE_MASS) {
			...
			*PageError = 0xFFFFFFFFU;
			...
			for (index = pEraseInit->Page;
					index < (pEraseInit->Page + pEraseInit->NbPages); index++) {
				/* Start erase page */
...
HAL_StatusTypeDef HAL_FLASHEx_OBProgram(FLASH_OBProgramInitTypeDef *pOBInit) {
	...
	if ((pOBInit->OptionType & OPTIONBYTE_WRP) != 0x00U) {
		...
		FLASH_OB_WRPConfig(pOBInit->WRPArea, pOBInit->WRPStartOffset,
				pOBInit->WRPEndOffset);
	}

	/* Option register */
	if ((pOBInit->OptionType & (OPTIONBYTE_RDP | OPTIONBYTE_USER))
			== (OPTIONBYTE_RDP | OPTIONBYTE_USER)) {
	...
	if ((pOBInit->OptionType & OPTIONBYTE_PCROP) != 0x00U) {
		...
		if ((pOBInit->PCROPConfig & (OB_PCROP_ZONE_A | OB_PCROP_RDP_ERASE))
				!= 0x00U) {
			/* Configure the 1A Proprietary code readout protection */
			FLASH_OB_PCROP1AConfig(pOBInit->PCROPConfig,
					pOBInit->PCROP1AStartAddr, pOBInit->PCROP1AEndAddr);
		}

		if ((pOBInit->PCROPConfig & OB_PCROP_ZONE_B) != 0x00U) {
			/* Configure the 1B Proprietary code readout protection */
			FLASH_OB_PCROP1BConfig(pOBInit->PCROP1BStartAddr,
					pOBInit->PCROP1BEndAddr);
		}

#if defined(FLASH_DBANK_SUPPORT)
		if ((pOBInit->PCROPConfig & OB_PCROP_ZONE2_A) != 0x00U) {
			/* Configure the 2A Proprietary code readout protection */
			FLASH_OB_PCROP2AConfig(pOBInit->PCROP2AStartAddr,
					pOBInit->PCROP2AEndAddr);
		}

		if ((pOBInit->PCROPConfig & OB_PCROP_ZONE2_B) != 0x00U) {
			/* Configure the 2B Proprietary code readout protection */
			FLASH_OB_PCROP2BConfig(pOBInit->PCROP2BStartAddr,
					pOBInit->PCROP2BEndAddr);
		}
#endif /* FLASH_DBANK_SUPPORT */
	}
#endif /* FLASH_PCROP_SUPPORT */
...
void HAL_FLASHEx_OBGetConfig(FLASH_OBProgramInitTypeDef *pOBInit) {
	...
	FLASH_OB_GetPCROP1A(&(pOBInit->PCROPConfig), &(pOBInit->PCROP1AStartAddr),
			&(pOBInit->PCROP1AEndAddr));
	FLASH_OB_GetPCROP1B(&(pOBInit->PCROP1BStartAddr),
			&(pOBInit->PCROP1BEndAddr));
	pOBInit->PCROPConfig |= (OB_PCROP_ZONE_A | OB_PCROP_ZONE_B);
#if defined(FLASH_DBANK_SUPPORT)
	FLASH_OB_GetPCROP2A(&(pOBInit->PCROP2AStartAddr),
			&(pOBInit->PCROP2AEndAddr));
	FLASH_OB_GetPCROP2B(&(pOBInit->PCROP2BStartAddr),
			&(pOBInit->PCROP2BEndAddr));
...
void FLASH_PageErase(uint32_t Banks, uint32_t Page) {
  ...
  UNUSED(Banks);
	...
	FLASH->CR = (tmp
			| (FLASH_CR_STRT | (Page << FLASH_CR_PNB_Pos) | FLASH_CR_PER));
}
...
static void FLASH_OB_WRPConfig(uint32_t WRPArea, uint32_t WRPStartOffset,
	...
	if (WRPArea == OB_WRPAREA_ZONE_A) {
		FLASH->WRP1AR = ((WRDPEndOffset << FLASH_WRP1AR_WRP1A_END_Pos)
				| WRPStartOffset);
	}
#if defined(FLASH_DBANK_SUPPORT)
	else if (WRPArea == OB_WRPAREA_ZONE2_A) {
		FLASH->WRP2AR = ((WRDPEndOffset << FLASH_WRP2AR_WRP2A_END_Pos)
				| WRPStartOffset);
	} else if (WRPArea == OB_WRPAREA_ZONE2_B) {
		FLASH->WRP2BR = ((WRDPEndOffset << FLASH_WRP2BR_WRP2B_END_Pos)
				| WRPStartOffset);
	}
#endif /* FLASH_DBANK_SUPPORT */
	else {
		FLASH->WRP1BR = ((WRDPEndOffset << FLASH_WRP1BR_WRP1B_END_Pos)
				| WRPStartOffset);
	}
}
...
static void FLASH_OB_PCROP1AConfig(uint32_t PCROPConfig,
	...
	if ((PCROPConfig & OB_PCROP_ZONE_A) != 0x00U) {
		...
		startoffset = ((PCROP1AStartAddr - ropbase)
				>> FLASH_PCROP_GRANULARITY_OFFSET);
		endoffset = ((PCROP1AEndAddr - ropbase)
				>> FLASH_PCROP_GRANULARITY_OFFSET);
...
static void FLASH_OB_PCROP1BConfig(uint32_t PCROP1BStartAddr,
	...
	/* Configure the Proprietary code readout protection offset */
	startoffset = ((PCROP1BStartAddr - ropbase)
			>> FLASH_PCROP_GRANULARITY_OFFSET);
...
static void FLASH_OB_GetPCROP1A(uint32_t *PCROPConfig,
	...
	*PCROP1AEndAddr = ((pcrop & FLASH_PCROP1AER_PCROP1A_END)
			<< FLASH_PCROP_GRANULARITY_OFFSET);
...
static void FLASH_OB_PCROP2AConfig(uint32_t PCROP2AStartAddr,
	...
	/* Configure the Proprietary code readout protection offset */
	startoffset = ((PCROP2AStartAddr - ropbase)
			>> FLASH_PCROP_GRANULARITY_OFFSET);
...
static void FLASH_OB_PCROP2BConfig(uint32_t PCROP2BStartAddr,
	...
	/* Configure the Proprietary code readout protection offset */
	startoffset = ((PCROP2BStartAddr - ropbase)
			>> FLASH_PCROP_GRANULARITY_OFFSET);
...
static void FLASH_OB_SecMemConfig(uint32_t BootEntry, uint32_t SecSize,
	...
	secmem = (FLASH->SECR
			& ~(FLASH_SECR_BOOT_LOCK | FLASH_SECR_SEC_SIZE
