...
void HAL_RCCEx_GetPeriphCLKConfig(RCC_PeriphCLKInitTypeDef *PeriphClkInit) {
	...
	PeriphClkInit->PeriphClockSelection = RCC_PERIPHCLK_USART1
			| RCC_PERIPHCLK_I2C1 | RCC_PERIPHCLK_I2S1 |
      ...
      case RCC_PERIPHCLK_RNG:
		...
		case RCC_PERIPHCLK_CEC:
			...
			if ((HAL_IS_BIT_SET(RCC->CR, RCC_CR_HSIRDY))
					&& (srcclk == RCC_CECCLKSOURCE_HSI_DIV488)) {
				frequency = (HSI_VALUE / 488U);
			} else if ((HAL_IS_BIT_SET(RCC->BDCR, RCC_BDCR_LSERDY))
					&& (srcclk == RCC_CECCLKSOURCE_LSE)) {
				frequency = LSE_VALUE;
			}
		...
		case RCC_PERIPHCLK_LPUART2:
			...
			if (srcclk == RCC_LPUART2CLKSOURCE_PCLK1) {
				...
				frequency = HAL_RCC_GetSysClockFreq();
					...
					&& (srcclk == RCC_LPUART2CLKSOURCE_HSI)) {
				frequency = HSI_VALUE;
			} else if ((HAL_IS_BIT_SET(RCC->BDCR, RCC_BDCR_LSERDY))
					&& (srcclk == RCC_LPUART2CLKSOURCE_LSE)) {
				frequency = LSE_VALUE;
			}
		...
		case RCC_PERIPHCLK_ADC:
			...
			if (srcclk == RCC_ADCCLKSOURCE_SYSCLK) {
				...
				if (__HAL_RCC_GET_PLLCLKOUT_CONFIG(RCC_PLLPCLK) != 0U) {
					...
					plln = READ_BIT(RCC->PLLCFGR,
							RCC_PLLCFGR_PLLN) >> RCC_PLLCFGR_PLLN_Pos;
					frequency = (pllvco * plln)
		...
		case RCC_PERIPHCLK_LPTIM2:
			...
			if (srcclk == RCC_LPTIM2CLKSOURCE_PCLK1) {
				frequency = HAL_RCC_GetPCLK1Freq();
					...
					&& (srcclk == RCC_LPTIM2CLKSOURCE_LSI)) {
				frequency = LSI_VALUE;
			} else if ((HAL_IS_BIT_SET(RCC->CR, RCC_CR_HSIRDY))
					&& (srcclk == RCC_LPTIM2CLKSOURCE_HSI)) {
				frequency = HSI_VALUE;
			} else if ((HAL_IS_BIT_SET(RCC->BDCR, RCC_BDCR_LSERDY))
					&& (srcclk == RCC_LPTIM2CLKSOURCE_LSE)) {
				frequency = LSE_VALUE;
			}
...
void HAL_RCCEx_CRSGetSynchronizationInfo(
	...
	pSynchroInfo->HSI48CalibrationValue = (READ_BIT(CRS->CR, CRS_CR_TRIM)
			>> CRS_CR_TRIM_Pos);

	/* Get Frequency error capture */
	pSynchroInfo->FreqErrorCapture = (READ_BIT(CRS->ISR, CRS_ISR_FECAP)
			>> CRS_ISR_FECAP_Pos);
