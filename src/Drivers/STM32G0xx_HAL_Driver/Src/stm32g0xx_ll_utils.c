...
ErrorStatus LL_PLL_ConfigSystemClock_HSI(
	...
	if (UTILS_PLL_IsBusy() == SUCCESS) {
		...
		LL_RCC_PLL_ConfigDomain_SYS(LL_RCC_PLLSOURCE_HSI,
				UTILS_PLLInitStruct->PLLM, UTILS_PLLInitStruct->PLLN,
...
static uint32_t UTILS_GetPLLOutputFrequency(uint32_t PLL_InputFrequency,
	...
	pllfreq = pllfreq
			/ (((UTILS_PLLInitStruct->PLLR >> RCC_PLLCFGR_PLLR_Pos) + 1U));
...
static ErrorStatus UTILS_EnablePLLAndSwitchSystem(uint32_t SYSCLK_Frequency,
	...
	hclk_frequency = __LL_RCC_CALC_HCLK_FREQ(SYSCLK_Frequency,
