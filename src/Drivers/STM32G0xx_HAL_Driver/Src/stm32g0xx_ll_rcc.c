...
void LL_RCC_GetSystemClocksFreq(LL_RCC_ClocksTypeDef *RCC_Clocks) {
	...
	RCC_Clocks->PCLK1_Frequency = RCC_GetPCLK1ClockFreq(
...
uint32_t LL_RCC_GetUSARTClockFreq(uint32_t USARTxSource) {
	...
	if (USARTxSource == LL_RCC_USART1_CLKSOURCE) {
		...
		default:
			usart_frequency = RCC_GetPCLK1ClockFreq(
...
uint32_t LL_RCC_GetLPUARTClockFreq(uint32_t LPUARTxSource) {
	...
	if (LPUARTxSource == LL_RCC_LPUART1_CLKSOURCE) {
		...
		default:
			lpuart_frequency = RCC_GetPCLK1ClockFreq(
					RCC_GetHCLKClockFreq(RCC_GetSystemClockFreq()));
			break;
		}
	}
...
uint32_t LL_RCC_GetLPTIMClockFreq(uint32_t LPTIMxSource) {
	...
	if (LPTIMxSource == LL_RCC_LPTIM1_CLKSOURCE) {
		...
		default:
			lptim_frequency = RCC_GetPCLK1ClockFreq(
					RCC_GetHCLKClockFreq(RCC_GetSystemClockFreq()));
			break;
		}
	} else {
...
uint32_t LL_RCC_GetTIMClockFreq(uint32_t TIMxSource) {
	...
	if (TIMxSource == LL_RCC_TIM1_CLKSOURCE) {
		...
		default:
			tim_frequency = RCC_GetPCLK1ClockFreq(
					RCC_GetHCLKClockFreq(RCC_GetSystemClockFreq()));
			break;
		}
	}
...
uint32_t LL_RCC_GetFDCANClockFreq(uint32_t FDCANxSource) {
	...
	default:
		fdcan_frequency = RCC_GetPCLK1ClockFreq(
				RCC_GetHCLKClockFreq(RCC_GetSystemClockFreq()));
		break;
	}
...
static uint32_t RCC_PLL_GetFreqDomain_FDCAN(void) {
	...
	return __LL_RCC_CALC_PLLCLK_FDCAN_FREQ(pllinputfreq,
			LL_RCC_PLL_GetDivider(), LL_RCC_PLL_GetN(), LL_RCC_PLL_GetQ());
}
#endif /* FDCAN1 || FDCAN2 */
...
static uint32_t RCC_PLL_GetFreqDomain_TIM15(void) {
	...
	return __LL_RCC_CALC_PLLCLK_TIM15_FREQ(pllinputfreq,
			LL_RCC_PLL_GetDivider(), LL_RCC_PLL_GetN(), LL_RCC_PLL_GetQ());
}
#endif /* RCC_CCIPR_TIM15SEL */
/**
