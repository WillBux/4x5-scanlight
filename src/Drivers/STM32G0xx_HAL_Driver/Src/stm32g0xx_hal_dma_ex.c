...
HAL_StatusTypeDef HAL_DMAEx_ConfigMuxSync(DMA_HandleTypeDef *hdma,
	...
	if (hdma->State == HAL_DMA_STATE_READY) {
		...
		MODIFY_REG(hdma->DMAmuxChannel->CCR, (~DMAMUX_CxCR_DMAREQ_ID),
				(pSyncConfig->SyncSignalID | ((pSyncConfig->RequestNumber - 1U) << DMAMUX_CxCR_NBREQ_Pos) | pSyncConfig->SyncPolarity | ((uint32_t)pSyncConfig->SyncEnable << DMAMUX_CxCR_SE_Pos) | ((uint32_t)pSyncConfig->EventEnable << DMAMUX_CxCR_EGE_Pos)));
...
HAL_StatusTypeDef HAL_DMAEx_ConfigMuxRequestGenerator(DMA_HandleTypeDef *hdma,
	...
	assert_param(
			IS_DMAMUX_REQUEST_GEN_REQUEST_NUMBER(pRequestGeneratorConfig->RequestNumber));
	...
	if (hdma->DMAmuxRequestGen == 0U) {
		...
		/* error status */
		status = HAL_ERROR;
	} else if (((hdma->DMAmuxRequestGen->RGCR & DMAMUX_RGxCR_GE) == 0U)
			&& (temp_state == HAL_DMA_STATE_READY)) {
...
void HAL_DMAEx_MUX_IRQHandler(DMA_HandleTypeDef *hdma) {
	...
	if ((hdma->DMAmuxChannelStatus->CSR & hdma->DMAmuxChannelStatusMask)
			!= 0U) {
