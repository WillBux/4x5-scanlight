...
DMA_Channel_TypeDef* USBPD_HW_Init_DMARxInstance(uint8_t PortNum) {
	...
	case 0:
		...
		(void) LL_DMA_Init(UCPDDMA_INSTANCE0_DMA_RX,
				UCPDDMA_INSTANCE0_LL_CHANNEL_RX, &DMA_InitStruct);
		break;
...
DMA_Channel_TypeDef* USBPD_HW_Init_DMATxInstance(uint8_t PortNum) {
	...
	case 0:
		...
		(void) LL_DMA_Init(UCPDDMA_INSTANCE0_DMA_TX,
				UCPDDMA_INSTANCE0_LL_CHANNEL_TX, &DMA_InitStruct);
		break;
#if defined(UCPD_INSTANCE1)
    case 1 :
