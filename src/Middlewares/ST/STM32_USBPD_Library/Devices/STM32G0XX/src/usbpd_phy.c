...
typedef struct {
	 ...
	 * @retval None
	 */
	void (*USBPD_PHY_MessageReceived)(uint8_t PortNum,
			USBPD_SOPType_TypeDef Type);
 ...
 * @{
 */
USBPD_StatusTypeDef PHY_PortInit(uint8_t PortNum,
		const USBPD_PHY_Callbacks *cbs, uint8_t *pRxBuffer,
 ...
 */
USBPD_StatusTypeDef USBPD_PHY_Init(uint8_t PortNum,
		const USBPD_PHY_Callbacks *pCallbacks, uint8_t *pRxBuffer,
		USBPD_PortPowerRole_TypeDef PowerRole, uint32_t SupportedSOP) {
	(void) PowerRole;

	/* set all callbacks */
	Ports[PortNum].cbs.USBPD_HW_IF_TxCompleted =
			pCallbacks->USBPD_PHY_TxCompleted;
	Ports[PortNum].cbs.USBPD_HW_IF_BistCompleted =
			pCallbacks->USBPD_PHY_BistCompleted;
	Ports[PortNum].cbs.USBPD_HW_IF_RX_ResetIndication =
			pCallbacks->USBPD_PHY_ResetIndication;
	Ports[PortNum].cbs.USBPD_HW_IF_RX_Completed = PHY_Rx_Completed;
	Ports[PortNum].cbs.USBPD_HW_IF_TX_HardResetCompleted =
			pCallbacks->USBPD_PHY_ResetCompleted;
	Ports[PortNum].cbs.USBPD_HW_IF_TX_FRSReception =
			pCallbacks->USBPD_PHY_FastRoleSwapReception;
	...
	PHY_Ports[PortNum].USBPD_PHY_MessageReceived =
			pCallbacks->USBPD_PHY_MessageReceived;
 ...
 * @retval status  @ref USBPD_OK
 */
USBPD_StatusTypeDef USBPD_PHY_ResetRequest(uint8_t PortNum,
		USBPD_SOPType_TypeDef Type) {
 ...
 * @retval status    @ref USBPD_OK
 */
USBPD_StatusTypeDef USBPD_PHY_SendMessage(uint8_t PortNum,
		USBPD_SOPType_TypeDef Type, uint8_t *pBuffer, uint16_t Size) {
 ...
 * @retval USBPD   status
 */
USBPD_StatusTypeDef USBPD_PHY_ExitTransmit(uint8_t PortNum,
		USBPD_SOPType_TypeDef mode) {
...
void PHY_Rx_Completed(uint8_t PortNum, uint32_t MsgType) {
	...
	USBPD_SOPTYPE_SOP1_DEBUG, USBPD_SOPTYPE_SOP2_DEBUG,
	...
	case USBPD_SOPTYPE_SOP2_DEBUG:
		if (!((uint8_t) (0x1u << _msgtype)
				!= (PHY_Ports[PortNum].SupportedSOP
						& (uint8_t) (0x1u << _msgtype)))) {
