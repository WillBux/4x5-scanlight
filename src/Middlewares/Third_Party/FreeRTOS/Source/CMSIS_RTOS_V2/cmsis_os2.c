...
uint32_t osThreadFlagsSet(osThreadId_t thread_id, uint32_t flags) {
	...
	if ((hTask == NULL) || ((flags & THREAD_FLAGS_INVALID_BITS) != 0U)) {
		...
		if (IS_IRQ()) {
			...
			(void) xTaskNotifyAndQueryFromISR(hTask, 0, eNoAction, &rflags,
					NULL);
...
osTimerId_t osTimerNew(osTimerFunc_t func, osTimerType_t type, void *argument,
	...
	if (!IS_IRQ() && (func != NULL)) {
		...
		if (callb != NULL) {
			...
			if (mem == 1) {
#if (configSUPPORT_STATIC_ALLOCATION == 1)
				hTimer = xTimerCreateStatic(name, 1, reload, callb,
						TimerCallback, (StaticTimer_t*) attr->cb_mem);
#endif
			} else {
				if (mem == 0) {
#if (configSUPPORT_DYNAMIC_ALLOCATION == 1)
					hTimer = xTimerCreate(name, 1, reload, callb,
							TimerCallback);
#endif
				}
			}
...
uint32_t osEventFlagsSet(osEventFlagsId_t ef_id, uint32_t flags) {
    ...
    rflags = (uint32_t)osErrorResource;
		...
		if (xEventGroupSetBitsFromISR(hEventGroup, (EventBits_t) flags,
				&yield) == pdFAIL) {
			rflags = (uint32_t) osErrorResource;
		} else {
			rflags = flags;
...
uint32_t osEventFlagsClear(osEventFlagsId_t ef_id, uint32_t flags) {
    ...
    rflags = (uint32_t)osErrorResource;
		...
		if (xEventGroupClearBitsFromISR(hEventGroup,
				(EventBits_t) flags) == pdFAIL) {
			rflags = (uint32_t) osErrorResource;
		}
#endif
	} else {
	...
	if (!IS_IRQ() && (max_count > 0U) && (initial_count <= max_count)) {
		...
		if (mem != -1) {
			if (max_count == 1U) {
				...
				if (mem == 1) {
#if (configSUPPORT_STATIC_ALLOCATION == 1)
					hSemaphore = xSemaphoreCreateCountingStatic(max_count,
							initial_count, (StaticSemaphore_t* )attr->cb_mem);
#endif
				} else {
#if (configSUPPORT_DYNAMIC_ALLOCATION == 1)
					hSemaphore = xSemaphoreCreateCounting(max_count,
							initial_count);
#endif
				}
			}
...
osMessageQueueId_t osMessageQueueNew(uint32_t msg_count, uint32_t msg_size,
	...
	if (!IS_IRQ() && (msg_count > 0U) && (msg_size > 0U)) {
		...
		if (mem == 1) {
#if (configSUPPORT_STATIC_ALLOCATION == 1)
			hQueue = xQueueCreateStatic(msg_count, msg_size, attr->mq_mem,
					attr->cb_mem);
#endif
		} else {
			if (mem == 0) {
