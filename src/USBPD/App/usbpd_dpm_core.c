...
void USBPD_DPM_CADCallback(uint8_t PortNum, USBPD_CAD_EVENT State,
		CCxPin_TypeDef Cc);
...
USBPD_StatusTypeDef USBPD_DPM_InitCore(void) {
	...
	NULL, USBPD_DPM_HardReset,
	NULL, USBPD_DPM_Notification, USBPD_DPM_ExtendedMessageReceived,
			USBPD_DPM_GetDataInfo, USBPD_DPM_SetDataInfo,
			NULL, USBPD_DPM_SNK_EvaluateCapabilities,
			NULL, USBPD_PE_TaskWakeUp,
			...
			USBPD_DPM_EnterErrorRecovery, USBPD_DPM_EvaluateDataRoleSwap,
			USBPD_DPM_IsPowerReady };

	static const USBPD_CAD_Callbacks CAD_cbs = { USBPD_DPM_CADCallback,
	...
	for (uint8_t _port_index = 0; _port_index < USBPD_PORT_COUNT;
		...
		/* CAD SET UP : Port 0 */
		CHECK_CAD_FUNCTION_CALL(
				USBPD_CAD_Init(_port_index, &CAD_cbs,
						&DPM_Settings[_port_index], &DPM_Params[_port_index]));

		/* PE SET UP : Port 0 */
		CHECK_PE_FUNCTION_CALL(
				USBPD_PE_Init(_port_index,
...
USBPD_StatusTypeDef USBPD_DPM_InitOS(void) {
	OS_INIT();
	{
		OS_CREATE_QUEUE(CADQueueId, "QCAD", USBPD_PORT_COUNT, OS_ELEMENT_SIZE); OS_DEFINE_TASK(CAD, USBPD_CAD_Task, OS_CAD_PRIORITY, OS_CAD_STACK_SIZE, NULL);
		OS_CREATE_TASK(CADThread, CAD, USBPD_CAD_Task, OS_CAD_PRIORITY,
				OS_CAD_STACK_SIZE, (int)NULL);
	}
	...
	for (uint32_t index = 0; index < USBPD_PORT_COUNT; index++) {
		...
		if (index == USBPD_PORT_0) {
			...
			OS_CREATE_TASK(DPM_PEThreadId_Table[USBPD_PORT_0], PE_0,
					USBPD_PE_Task, OS_PE_PRIORITY, OS_PE_STACK_SIZE,
					(int )index);
		}
